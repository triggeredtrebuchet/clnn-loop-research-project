...
def create_features(output_train = 'project_train.npy', output_dev = 'project_dev.npy', cell_line = 'project_data/', dev_left = 'data/project_data/dev_left', dev_right = 'data/project_data/dev_right',
    ...
    sites = [i for i in range(10, len(data[0]) - 10)]
    if data.ndim == 1:
    ...
    # sites = [i for i in range(k, len(data[0]) - k)]
    base = np.loadtxt(outf)
    pwm1 = PWM(base, k)  # load
